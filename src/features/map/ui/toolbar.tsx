...
const GROUP_ORDER: ShellToolbarAction['group'][] = ['data']

export function Toolbar({ actions, onAction }: ToolbarProps) {
  return (
    <div className='pointer-events-none absolute top-5 left-5 right-5 z-30 flex items-start justify-between gap-4'>
          ...
          {GROUP_ORDER.map((group, groupIndex) => {
            ...
            return (
              <div key={group} className='flex items-center gap-1.5'>
                {groupIndex > 0 ? <span className='mx-1 h-8 w-px bg-white/12' /> : null}
