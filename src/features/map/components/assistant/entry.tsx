...
function AssistantEdgeTrigger({ onOpen }: { onOpen: () => void }) {
  return (
    <button
      ...
      aria-label='打开对话详情'
    >
      <span className='absolute inset-y-0 left-3 w-px bg-transparent transition-colors duration-300 group-hover:bg-neutral-800' />
      <span className='absolute inset-0 bg-neutral-950/10 opacity-0 transition-opacity duration-300 group-hover:opacity-100' />
    </button>
  )
}
